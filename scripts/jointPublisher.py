...
class JointMapper():
    ...
    def set_j(self,limb, joint_names, angles,speed):
        ...
        joint_command = {joint_names[0]: angles[0],joint_names[1]: angles[1],joint_names[2]: angles[2], joint_names[3]: angles[3]}
    ...
    def callback(self,data):
        ...
        #For Shoulder Roll
        shoulderRoll_v0_1 = np.array([shoulder_left.x,shoulder_left.y,shoulder_left.z]) - np.array([shoulder_right.x,shoulder_right.y,shoulder_right.z])
        shoulderRoll_v1_1 = np.array([shoulder_left.x,shoulder_left.y,shoulder_left.z]) - np.array([elbow_left.x,elbow_left.y,elbow_left.z])
        
        shoulderRoll_v0_2 = np.array([elbow_left.x, elbow_left.y, elbow_left.z]) - np.array([wrist_left.x, wrist_left.y, wrist_left.z])
        shoulderRoll_v1_2 = np.array([elbow_left.x, elbow_left.y, elbow_left.z]) - np.array([shoulder_left.x,shoulder_left.y,shoulder_left.z])
        
        EM = np.cross(shoulderRoll_v0_1, shoulderRoll_v1_1) #perpedicular to plane DEA
        AH = np.cross(shoulderRoll_v0_2, shoulderRoll_v1_2) #perpendicular to plane EAB

        dot_product = np.dot(EM, AH)
        mag_v0 = np.linalg.norm(EM)
        mag_v1 = np.linalg.norm(AH)

        shoulderRoll_angle = (180/math.pi)*np.arccos(1.0*(dot_product)/(mag_v0*mag_v1))
        #angle_list.append(elbowPitch_angle)
        ...
        #######################
        b_shoulderRoll = 2.606616 - 0.02124562*(elbowPitch_angle-55) #Find the scale and offset
        #######################
        ...
        #######################
        del_shoulder_roll=abs(b_shoulderRoll-prev_angles[0][2]) #change index  
        #######################      
        del_elbow_pitch=abs(b_elbowPitch-prev_angles[0][3])
	...
	if (del_elbow_pitch<0.7):
		...
		prev_angles[0][3]=b_elbowPitch
		f.data[3] = b_elbowPitch
		stable_angles[0][3] = b_elbowPitch
	else:
		#self.set_j(self.left,self.lj[3],(prev_angles[0][2]),speed);
		setElbowPitch = stable_angles[0][3]
		prev_angles[0][3] = b_elbowPitch
		f.data[3] = stable_angles[0][3]

        ###############################
        ###############################
    
    if (del_shoulder_roll<0.7):
        #self.set_j(self.left,self.lj[3],(b_elbowPitch),speed);
        setShoulderRoll = b_shoulderRoll
        prev_angles[0][2]=b_shoulderRoll
        f.data[2] = b_shoulderRoll
        stable_angles[0][2] = b_shoulderRoll
    else:
        #self.set_j(self.left,self.lj[3],(prev_angles[0][2]),speed);
        setShoulderRoll = stable_angles[0][2]
        prev_angles[0][2] = b_shoulderRoll
        f.data[2] = stable_angles[0][2]

	self.set_j(self.left,[self.lj[0],self.lj[1],self.lj[2], self.lj[3]],[setShoulderYaw, setShoulderPitch, setShoulderRoll, setElbowPitch],speed);
